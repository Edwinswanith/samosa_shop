...
export function LpgAndSalaryPanel() {
  ...
  const activeStaff = useMemo(() => state.staff.filter((member) => member.status === "Active"), [state.staff]);
  const latestPeriodEnd = state.staffPayments.map((payment) => payment.periodEnd).sort().at(-1);
  const latestPayments = state.staffPayments.filter((payment) => payment.periodEnd === latestPeriodEnd);
  const latestPaid = latestPayments.reduce((total, payment) => total + Number(payment.amount), 0);
