...
import { calculateDashboard, calculateInventory, calculatePeriodSummary, calculateReceivable } from "@/domain/ledger";
...
function TransactionList({ transactions }: { transactions: ShopTransaction[] }) {
  const { state } = useShopStore(); return <div className="transactionList">{transactions.map((entry) => { let title = "Shop activity", detail = entry.businessDate, amount = ""; let positive = false;
    if (entry.kind === "sale") { title = state.products.find((p) => p.id === entry.productId)?.name ?? "Sale"; detail = `${entry.channel} · ${entry.quantity} sold`; amount = `+${formatMoney(entry.revenue)}`; positive = true; }
